...
st.markdown(
    "<div class='hero'><h1>🧠 Enterprise Workforce Intelligence</h1>"
    "<p>From workforce risk to capability preservation: retain, reskill, or hire.</p></div>",
    unsafe_allow_html=True,
)
...
high_risk_mask = probabilities >= 0.70
medium_risk_mask = (probabilities >= 0.40) & (probabilities < 0.70)
...
# The skill taxonomy is role-based. It is not presented as certified employee skill data.
...
filtered_probs = model.predict_proba(filtered[MODEL_FEATURES])[:, 1]
filtered_high_risk = int((filtered_probs >= risk_threshold).sum())
...
st.caption(
    f"Model: class-balanced Logistic Regression | Holdout ROC-AUC: {metrics['roc_auc']:.3f} | "
    f"Global high-risk count at 70%: {high_risk_count}"
)
    ...
    risk_mix = pd.Series(
        {
            ...
            "Low": len(df) - high_risk_count - medium_risk_count,
        }
    )
...
# Inferred capability profile is deliberately labelled as MVP inference.
current_skills = []
...
with right:
    ...
    st.write(f"**Decision rationale:** {result['recommendation']['evidence']}")
...
query = st.text_input(
    "Search for a related capability",
    value=(result["skills"]["skill_gap"][0] if result["skills"]["skill_gap"] else "Machine Learning"),
)
...
if st.button("Find Semantically Related Skills"):
    try:
        ...
        st.info("Core workforce decision logic remains available without transformer inference.")
...
if llm.enabled:
    ...
    st.info("Optional: set OPENAI_API_KEY and OPENAI_MODEL to enable the LLM explanation agent.")

with st.expander("Agent trace / audit evidence"):
    st.json(result)

st.caption(
    "Responsible-use boundary: risk and recommendations are decision-support signals. "
    "Human HR review is required before action; sensitive demographic attributes are not used by the recommendation layer."
)
